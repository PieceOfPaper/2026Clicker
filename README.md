...
| 재화 | 획득 방법 | 사용처 | 비고 |
| --- | --- | --- | --- |
| Gold(골드) | 일반 몬스터 및 보스 처치 | 캐릭터 공격력, 치명타 확률, 치명타 대미지, 향후 자동 전투 요소 강화 | 몬스터 처치 시 기본 드롭 |
...
- 강화 후보는 캐릭터 공격력, 치명타 확률, 치명타 대미지와 향후 추가할 동료 또는 자동 전투 요소다.
- 현재 MVP에서는 캐릭터 공격력 강화를 가장 먼저 구현한다.
- 치명타와 자동 전투 요소의 해금 조건, 강화 단계 및 비용 곡선은 추후 결정한다.
...
- 몬스터 처치로 획득한 Gold를 소비해 전투 관련 능력을 강화한다.
- 첫 구현 대상은 캐릭터 공격력이며, 이후 치명타 확률과 치명타 대미지를 추가 후보로 둔다.
- 동료라는 명칭과 구조는 미정이지만, 자동 공격을 담당하는 요소도 Gold 강화 대상으로 고려한다.
- 각 강화 항목은 현재 레벨, 현재 효과, 다음 효과와 Gold 비용을 표시한다.
...
| 2026-08-23 | 몬스터 처치 시 기본으로 Gold를 드롭하고, 캐릭터 공격력·치명타·향후 자동 전투 요소 강화에 사용한다. | 처치와 강화가 연결되는 기본 성장 순환을 명확히 하기 위해 |
