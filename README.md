...
| 터치 치명타 대미지 | 기본 `100%`에 레벨당 `0.01%p` 가산 | `100.01%` | 5000레벨, `150%` |
...
= 100 + TouchCriticalDamageBonusPercent
```
...
- `100%`는 터치 피해와 동일한 피해를 뜻하고 `150%`는 터치 피해의 1.5배를 뜻한다.
...
| 터치 치명타 대미지 | 기본 100% + 레벨당 0.01%p, 1레벨 100.01%, 5000레벨 150% |
...
| 2026-08-23 | 치명타 확률과 치명타 대미지는 레벨당 0.01%p씩 성장하며 5000레벨에서 각각 50%와 총 150%가 된다. | 초기 밸런싱에 사용할 임시 선형 성장 기준을 마련하기 위해 |
