...
| --- | --- | --- | --- |
| `ID` | `int` | 강화 레벨 | 1~5000레벨 |
| `BonusDamageRate` | `float` | 기본 100%에 더할 치명타 대미지 보너스 | 0.0~1.0 값, 1레벨 0.0001, 5000레벨 0.5 |
| `RequiredGold` | `string` | 이전 레벨에서 해당 레벨로 강화할 때 필요한 Gold | BigNumber로 파싱, 1레벨은 0 |

```text
최종 치명타 배율 = 1.0 + BonusDamageRate
```

- 테스트 데이터는 1~10레벨과 상한 검증용 5000레벨을 제공한다.
- `BonusDamageRate=0.0001`이면 최종 배율은 `1.0001`, 즉 `100.01%`다.
- `BonusDamageRate=0.5`이면 최종 배율은 `1.5`, 즉 `150%`다.
- 비용은 임시값이며 최종 비용 곡선은 미정이다.
## CurrencyTable 후보
